...
const NONE: OrderResolution = { target: null, marchTo: null, disengage: false };
...
export function resolveRoyalOrder(u: IUnit): OrderResolution {
  ...
  if (order.kind === 'move') {
    ...
    return { target: null, marchTo: order.x, disengage: traveling };
  }

  const f = order.target;
  if (!f || f.dead || f.side === u.side) {
    u.order = null; // focus gone → resume autonomous
    return NONE;
  }
  const cf = f.x + f.unitW / 2;
  const gap = Math.abs(cf - cu) - (f.unitW + u.unitW) / 2;
  if (gap <= u.range) return { target: f, marchTo: null, disengage: false };
  return { target: null, marchTo: cf, disengage: true };
}
