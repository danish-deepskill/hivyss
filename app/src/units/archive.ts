// ARCHIVED — the legacy "military alpha" units (Grunt → Centurion). These were
// the original combat-test roster; α is now α Primal (`units/alpha.ts`). Kept
// registered under the `archive` geneline (so the 600+ combat tests that
// reference them still run) — slated to re-home into δ Military when it's built.
import type { UnitDef, UnitModule } from "../types";
import { PALETTES } from "../config/Palettes";
import drawGrunt from "../draws/archive/grunt";
import drawMandible from "../draws/archive/mandible";
import drawBombardier from "../draws/archive/bombardier";
import drawNeedler from "../draws/archive/needler";
import drawLegionnaire from "../draws/archive/legionnaire";
import drawRavager from "../draws/archive/ravager";
import drawCenturion from "../draws/archive/centurion";

const archiveDef = (def: Omit<UnitDef, "palette" | "geneline">): UnitDef => ({
  ...def,
  palette: PALETTES.alpha,
  geneline: "archive",
});

const gruntDef = archiveDef({
  name: "Grunt",
  ico: "\u{1F6E1}\uFE0F",
  hp: 90,
  atk: 20,
...
const bombardierDef = archiveDef({
  name: "Bombardier",
  ico: "\u{1F4A5}",
  hp: 100,
  atk: 28,
...
const legionnaireDef = archiveDef({
  name: "Legionnaire",
  ico: "\u{1FAB2}",
  hp: 800,
  atk: 18,
