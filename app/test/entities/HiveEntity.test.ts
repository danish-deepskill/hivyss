// Phase 6 follow-up #2 — HiveEntity wrapper tests.
//
...
//
// We use a stub HiveStructure rather than the real one (real
// HiveStructure extends Phaser.Container and needs a Scene). The
// stub satisfies the structural contract HiveEntity actually reads
// from (`hp`, `maxHp`, `setHp`, `flash`, `side`).

import { describe, it, expect, beforeEach } from 'vitest';
import { HiveEntity } from '../../src/entities/structures/HiveEntity';
import { resetStructureUid } from '../../src/entities/structures/StructureEntity';
import type { HiveStructure } from '../../src/entities/structures/HiveStructure';
import type { Side } from '../../src/types';

// Stub HiveStructure satisfying the fields HiveEntity reads.
function makeStubBase(side: Side, hp: number = 1000): HiveStructure {
  const stub = {
    side,
    hp,
    maxHp: hp,
...
describe('HiveEntity — WorldEntity contract', () => {
  it('exposes id, x, y, dead, components', () => {
    const stub = makeStubBase('player');
    const e = new HiveEntity(stub, 80);
  ...
  it('uses a negative id space (does not collide with Unit ids)', () => {
    const a = new HiveEntity(makeStubBase('player'), 80);
    const b = new HiveEntity(makeStubBase('enemy'), 920);
  ...
  it('has exactly the locked component set: HasHP, IsTargetable, HasAllegiance', () => {
    const e = new HiveEntity(makeStubBase('player'), 80);
...
describe('HiveEntity — SpatialEntity contract', () => {
  ...
  it('mirrors structure.side', () => {
    const p = new HiveEntity(makeStubBase('player'), 80);
    const en = new HiveEntity(makeStubBase('enemy'), 920);
...
describe('HiveEntity — damage delegation', () => {
  ...
  it('takeDamage clamps to 0 (HiveStructure.setHp clamps internally)', () => {
...
describe('HiveEntity — dead synchronization', () => {
  it('dead is false on a fresh entity', () => {
    const e = new HiveEntity(makeStubBase('player', 100), 80);
    expect(e.dead).toBe(false);
  });
